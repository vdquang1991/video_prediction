...
if opt.model_dir != '':
    ...
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    name = 'model=%s%dx%d-rnn_size=%d-predictor-posterior-rnn_layers=%d-%d-n_past=%d-n_future=%d-lr=%.4f-g_dim=%d-z_dim=%d-last_frame_skip=%d-beta=%.7f%s' % (
...
for epoch in range(opt.niter):
    ...
    print('Epoch [%02d]: val psnr: %.5f | val ssim: %.5f | val lpips: %.5f' % (epoch, psnr_value, ssim_value, lpips_value))
