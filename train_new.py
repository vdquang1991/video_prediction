...
from models.model_new import Encoder, Decoder, conv_lstm, gaussian_conv_lstm
import utils
...
def get_models(opt, device):
    ...
    encoder = Encoder(opt.g_dim, init_filters=opt.init_filters, n_channel=opt.n_channel * 2)
    decoder = Decoder(opt.g_dim, n_channel=opt.n_channel * 2, skip_type=opt.skip_type)
    frame_predictor = conv_lstm(opt.g_dim + opt.z_dim, opt.g_dim, opt.rnn_size, opt.predictor_rnn_layers, opt.batch_size, (4,4), device)
    posterior = gaussian_conv_lstm(opt.g_dim, opt.z_dim, opt.rnn_size, opt.posterior_rnn_layers, opt.batch_size, (4,4), device)
    prior = gaussian_conv_lstm(opt.g_dim, opt.z_dim, opt.rnn_size, opt.prior_rnn_layers, opt.batch_size, (4,4), device)
...
# --------- training funtions ------------------------------------
def train_step(x, models, optims, mse_criterion, kl_criterion, gdl_criterion, bce_criterion, opt, device):
    ...
    loss = mse + opt.beta_gdl * gdl + opt.beta_bce * bce + opt.beta_kld * kld
    loss.backward()
...
def training(opt, dataset_root, train_dataset, test_dataset, save_path='./save_path', training_steps=1000, test_steps=500, device='cpu'):
    ...
    all_metrics = {'train_loss': [], 'train_mse_loss': [], 'train_bce_loss': [], 'train_gdl_loss': [], 'train_kld_loss': [],
                   'psnr': [], 'ssim': [], 'lpips': []}
    best_psnr_value = 0.
    best_ssim_value = 0.
    ...
    for epoch in range(opt.start_epoch, opt.epochs):
        ...
        for i in range(training_steps):
            ...
            loss, mse, gdl, bce, kld = train_step(x, models, optims, mse_criterion, kl_criterion, gdl_criterion, bce_criterion, opt, device)
            epoch_loss += loss
            epoch_mse += mse
            epoch_gdl_loss += gdl
            epoch_bce_loss += bce
        ...
        print('Epoch [%03d]: train loss: %.5f | mse loss: %.5f | gdl loss: %.5f | bce loss: %.5f | kld loss: %.5f | training time: %.3f (%d)' % (
        epoch, epoch_loss / training_steps, epoch_mse / training_steps, epoch_gdl_loss /training_steps, epoch_bce_loss / training_steps, epoch_kld_loss / training_steps,
