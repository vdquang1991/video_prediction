...
from models.model_LMC_modify import Encoder, Decoder, conv_lstm, gaussian_conv_lstm
import utils
...
def get_models(opt, device):
    ...
    encoder = Encoder(opt.g_dim, init_filters=opt.init_filters, n_channel=opt.n_channel * 2, activation_type=opt.act)
    decoder = Decoder(opt.g_dim, init_filters=opt.init_filters, n_channel=opt.n_channel * 2, skip_type=opt.skip_type, activation_type=opt.act)
    img_dim = opt.image_size // 8
    frame_predictor = conv_lstm(opt.g_dim + opt.z_dim, opt.g_dim, opt.rnn_size, opt.predictor_rnn_layers, opt.batch_size, (img_dim, img_dim), device)
    posterior = gaussian_conv_lstm(opt.g_dim, opt.z_dim, opt.rnn_size, opt.posterior_rnn_layers, opt.batch_size, (img_dim, img_dim), device)
    prior = gaussian_conv_lstm(opt.g_dim, opt.z_dim, opt.rnn_size, opt.prior_rnn_layers, opt.batch_size, (img_dim, img_dim), device)
...
def eval_step(x, models, opt, device, lpips_dist):
    frame_predictor = models['frame_predictor']
    posterior = models['posterior']
    prior = models['prior']
    encoder = models['encoder']
    ...
    with torch.no_grad():
        ...
        lpips_score = utils.cal_lpips(gen_seq, gt_seq, lpips_dist, opt.n_channel, opt.image_size)
        results = {
            'mse': mse,
            'psnr': pnsr_score,
            'ssim': ssim_score,
...
def training(opt, dataset_root, train_dataset, test_dataset, save_path='./save_path', training_steps=1000, test_steps=500, device='cpu'):
    ...
    lpips_dist = lpips.LPIPS(net='alex')
    ...
    for epoch in range(opt.start_epoch, opt.epochs):
        ...
        for i in range(test_steps):
            ...
            results = eval_step(x, models, opt, device, lpips_dist)
